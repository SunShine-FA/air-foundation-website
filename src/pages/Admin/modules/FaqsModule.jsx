...
import { Plus, Edit2, Trash2, HelpCircle, GripVertical, ChevronUp, ChevronDown } from 'lucide-react';
import CrudModal from '../components/CrudModal';
import DeleteConfirmModal from '../components/DeleteConfirmModal';
import { contentService } from '../../../services/contentService';

export default function FaqsModule({ faqs = [], onRefresh, showToast }) {
  ...
  const fields = [
    { name: 'question', label: 'Question', placeholder: 'e.g. What is the teacher-to-student ratio?', required: true },
    { name: 'answer', label: 'Answer', type: 'textarea', rows: 4, required: true },
    {
      name: 'category',
      label: 'Category',
      type: 'select',
      options: ['General', 'Admissions', 'Academics', 'Facilities', 'Activities'],
  ...
  const handleDrop = async (e, targetIndex) => {
    ...
    setFaqList(withUpdatedOrder);
    setDraggedIdx(null);
    setDragOverIdx(null);
    await saveNewOrder(withUpdatedOrder, movedItem.question);
  };
  ...
  const handleMove = async (currentIndex, direction) => {
    ...
    setFaqList(withUpdatedOrder);
    await saveNewOrder(withUpdatedOrder, temp.question);
  };
  ...
  const saveNewOrder = async (orderedList, itemName) => {
    ...
    try {
      ...
      if (payload.length > 0) {
        const res = await contentService.reorder('faqs', payload);
        if (res.success) {
          showToast(`Reordered "${itemName.slice(0, 30)}...". Order saved!`, 'success');
        } else {
          showToast(res.error || 'Failed to save order in database', 'error');
        }
      } else {
        showToast(`Reordered locally`, 'success');
      }
      onRefresh();
    } catch (err) {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (editingItem?.id) {
        const res = await contentService.update('faqs', editingItem.id, formData);
        if (res.success) {
          showToast('FAQ updated successfully', 'success');
        } else {
          showToast(res.error || 'Failed to update FAQ', 'error');
        }
      } else {
        const res = await contentService.create('faqs', formData);
        if (res.success) {
          showToast('FAQ added successfully', 'success');
        } else {
          showToast(res.error || 'Failed to add FAQ', 'error');
        }
      }
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await contentService.remove('faqs', deleteItem.id);
      if (res.success) {
  ...
  return (
    <div className="space-y-6 text-left max-w-5xl">
        ...
        <div>
          <div className="flex items-center space-x-2.5">
            <h2 className="text-xl font-bold font-poppins text-slate-900">
              Frequently Asked Questions (FAQs) ({faqList.length})
            </h2>
            {isReordering && (
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Drag questions or use arrows to rearrange display order.
          </p>
        </div>
        <button
        ...
        {faqList.map((faq, idx) => {
          ...
          return (
            <div
              ...
              className={`bg-white p-5 rounded-2xl border shadow-sm flex flex-col justify-between transition-all duration-200 text-left space-y-3 select-none ${
                isDragging
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start space-x-3 flex-1 min-w-0">
                  ...
                  <div className="space-y-1 flex-1 min-w-0">
                    ...
                    <h3 className="font-bold text-base text-slate-800 font-poppins pt-0.5">{faq.question}</h3>
      ...
      <CrudModal
        ...
        fields={fields}
        initialData={editingItem || { category: 'General', sort_order: (faqList.length + 1) }}
        isSaving={isSaving}
      />
