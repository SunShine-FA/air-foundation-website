...
import { Plus, Edit2, Trash2, Newspaper, Calendar, GripVertical, ChevronLeft, ChevronRight } from 'lucide-react';
import CrudModal from '../components/CrudModal';
import DeleteConfirmModal from '../components/DeleteConfirmModal';
import { contentService } from '../../../services/contentService';

export default function NewsModule({ news = [], onRefresh, showToast }) {
  ...
  const fields = [
    { name: 'title', label: 'News Article Title', placeholder: 'e.g. National Robofest Championship 2026', required: true },
    {
      name: 'category',
      label: 'News Category',
      type: 'select',
      options: ['Achievement', 'Event', 'Academics', 'General', 'Sports'],
      defaultValue: 'Achievement',
      required: true
    },
    { name: 'date', label: 'Publication Date', type: 'date', placeholder: 'Pick publication date', required: true },
    { name: 'excerpt', label: 'Brief Excerpt / Summary', type: 'textarea', rows: 3, required: true },
    { name: 'image', label: 'Article Featured Image', type: 'image', required: true },
    { name: 'content', label: 'Full Article Details (Optional)', type: 'textarea', rows: 5, required: false },
  ...
  const saveNewOrder = async (orderedList, itemName) => {
    ...
    try {
      ...
      if (payload.length > 0) {
        const res = await contentService.reorder('news', payload);
        if (res.success) {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (editingItem?.id) {
        const res = await contentService.update('news', editingItem.id, formData);
        if (res.success) {
          showToast('News article updated successfully', 'success');
        } else {
          showToast(res.error || 'Failed to update news article', 'error');
        }
      } else {
        const res = await contentService.create('news', formData);
        if (res.success) {
          showToast('News article published successfully', 'success');
        } else {
          showToast(res.error || 'Failed to publish news article', 'error');
        }
      }
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await contentService.remove('news', deleteItem.id);
      if (res.success) {
        showToast('News article deleted', 'success');
        setDeleteItem(null);
        onRefresh();
      } else {
        showToast(res.error || 'Failed to delete article', 'error');
      }
    } catch (err) {
      showToast(err.message, 'error');
    } finally {
  ...
  return (
    <div className="space-y-6 text-left max-w-5xl">
        ...
        <div>
          <div className="flex items-center space-x-2.5">
            <h2 className="text-xl font-bold font-poppins text-slate-900">
              News & Bulletins ({newsList.length})
            </h2>
            {isReordering && (
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Drag cards or use arrows to rearrange news display order.
          </p>
        </div>
        <button
            ...
            >
              <div className="relative h-44 bg-slate-100 overflow-hidden group">
                <img
                  ...
                  onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1561557944-6e7860d1a7eb?auto=format&fit=crop&q=80&w=600'; }}
                />
                <div
                ...
                <span className="absolute top-3 right-3 bg-secondary text-slate-950 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md shadow-md">
                  {item.category}
                </span>
              </div>
              ...
              <div className="p-5 flex-grow flex flex-col justify-between space-y-4">
                <div>
                  ...
                  <h3 className="font-bold text-base text-slate-800 font-poppins line-clamp-2 leading-snug">
                    {item.title}
                  </h3>
      ...
      <CrudModal
        ...
        onSave={handleSave}
        title={editingItem ? 'Edit News Bulletin' : 'Publish News Bulletin'}
        fields={fields}
        initialData={editingItem || {
          category: 'Achievement',
          date: new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
          sort_order: (newsList.length + 1)
        }}
        isSaving={isSaving}
      />
